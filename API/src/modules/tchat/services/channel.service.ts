...
export class ChannelService {
	...
	async createChannel(channel: IChannel, creator: User): Promise<IChannel> {
		let { name, isPublic, password } = channel;
	...
	async deleteChannel(channel: IChannel) {
		const channelFound: Channel = await this.channelRepository.findOne({ where: { channelId: channel.channelId } });
		if (channelFound) {
			try {
				channelFound.users = [];
				try {
	...
	async getUser(client: Socket): Promise<User> {
		...
		const payload: JwtPayload = this.jwtService.verify(jwt, {secret: process.env.SECRET});
